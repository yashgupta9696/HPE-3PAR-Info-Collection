...
class display:
	...
	def display_function(self):
		...
		# if(not(len(self.ip_real) == len(self.user_real) == len(self.passwrd_real))):
		# 	messagebox.showerror("ERROR", "Please enter the credentials properly across rows!!")
		...
		def save_values_f2(event):
			...
			self.frame2_returned_values.append(var4.get())
			self.frame2_returned_values.append(var5.get())
			self.frame2_returned_values.append(var6.get())
			self.frame2_returned_values.append(var7.get())
			self.frame2_returned_values.append(var8.get())
			close_GUI(frame2)
			root.destroy()
		...
		Checkbutton(frame2, text="Virtual Volume Information", variable=var4).grid(row=4, column=1, pady=2, sticky=W)
		Checkbutton(frame2, text="Cage Information", variable=var5).grid(row=5, column=1, pady=2, sticky=W)
		Checkbutton(frame2, text="System Information", variable=var6).grid(row=6, column=1, pady=2, sticky=W)
		Checkbutton(frame2, text="Host Information", variable=var7).grid(row=7, column=1, pady=2, sticky=W)
		Checkbutton(frame2, text="Other Information", variable=var8).grid(row=8, column=1, pady=2, sticky=W)


		submit_f2 = ttk.Button(frame2, text="Submit")
		submit_f2.grid(row=9, column=1, sticky=W)
